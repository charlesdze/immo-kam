...
@section('content')
<div style="max-width: 1200px; margin: 0 auto; padding: 20px; animation: fadeInPage 0.8s ease-out;">
    
    {{-- HEADER STRATÉGIQUE --}}
    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 40px; background: white; padding: 30px; border-radius: 12px; border: 1px solid #e0e0e0; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
        <div>
            <h1 style="font-size: 2.5em; font-weight: 900; color: #2c3e50; margin: 0; text-transform: uppercase; letter-spacing: -1px;">
        ...
        <a href="{{ route('listings.create') }}" 
           ...
           style="background-color: #16a085; color: white; padding: 18px 35px; border-radius: 6px; font-weight: 800; text-decoration: none; box-shadow: 0 5px 0 #0e6655; transition: all 0.2s; font-size: 0.85em; text-transform: uppercase; letter-spacing: 1px;">
    ...
    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 25px; margin-bottom: 40px;">
        ...
        {{-- KPI MESSAGERIE : DEVIENT UN LIEN VERS LA PAGE MESSAGERIE --}}
    ...
    <div style="background: white; border-radius: 12px; border: 1px solid #ddd; box-shadow: 0 15px 40px rgba(0,0,0,0.08); overflow: hidden;">
        ...
        @if($myListings->isEmpty())
            <div style="padding: 100px; text-align: center; background: #fdfdfd;">
                        ...
                        <tr class="table-row" style="border-bottom: 1px solid #eee; transition: 0.2s;">
                            <td style="padding: 25px 35px;">
                                <div style="display: flex; align-items: center; gap: 20px;">
                                    ...
                                    <div>
                                        <p style="font-weight: 900; color: #2c3e50; margin: 0; font-size: 1.1em; text-transform: uppercase;">{{ $listing->title }}</p>
                            ...
                            <td style="padding: 25px 35px; text-align: right;">
                                <div style="display: flex; justify-content: flex-end; gap: 10px;">
                                    ...
                                    <form action="{{ route('admin.listings.destroy', $listing->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('ALERTE : Confirmation de suppression définitive ?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" style="background: #e74c3c; color: white; border: none; padding: 8px 15px; border-radius: 4px; font-weight: 800; font-size: 0.7em; cursor: pointer; text-transform: uppercase; box-shadow: 0 3px 0 #c0392b;">
                                            Destituer
                                        </button>
                                    </form>
                                </div>
